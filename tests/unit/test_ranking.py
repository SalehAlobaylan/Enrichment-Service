...
def test_freshness_decay_keeps_unaged_items_unchanged(settings) -> None:
    items = [_item("a", 1.0, "TWEET")]  # no published_at
...
def test_freshness_decay_decays_old_items(settings) -> None:
    ...
    items = [_item("old", 1.0, "TWEET", age_days=4)]
    decayed = ranking.apply_freshness_decay(items, settings)
...
def test_freshness_decay_reorders_by_decayed_score(settings) -> None:
    ...
    items = [
        _item("old_high", 1.0, "TWEET", age_days=4),
        _item("fresh_low", 0.5, "TWEET", age_days=0),
    ]
    decayed = ranking.apply_freshness_decay(items, settings)
...
def test_freshness_decay_uses_type_specific_tau(settings) -> None:
    ...
    article = _item("article", 1.0, "ARTICLE", age_days=30)
    tweet = _item("tweet", 1.0, "TWEET", age_days=30)
...
def test_source_diversity_caps_per_source() -> None:
    items = [
        _item(f"item_{i}", 1.0 - i * 0.01, "TWEET", source="acme")
        for i in range(5)
    ]
...
def test_source_diversity_preserves_rank() -> None:
    items = [
        _item("a", 0.9, "TWEET", source="src_x"),
        _item("b", 0.8, "TWEET", source="src_y"),
        _item("c", 0.7, "TWEET", source="src_x"),
        _item("d", 0.6, "TWEET", source="src_x"),  # 3rd from src_x → dropped
    ]
    kept = ranking.apply_source_diversity(items, max_per_source=2)
...
def test_source_diversity_doesnt_cap_unknown_source() -> None:
    items = [
        _item("a", 0.9, "TWEET", source=None),
        _item("b", 0.8, "TWEET", source=None),
        _item("c", 0.7, "TWEET", source=None),
    ]
...
def test_source_diversity_max_zero_returns_all() -> None:
    items = [_item("a", 0.9, "TWEET", source="acme")]
...
def test_type_quotas_interleaves_required_types() -> None:
    items = [
        _item("t1", 0.9, "TWEET"),
        _item("t2", 0.8, "TWEET"),
        _item("c1", 0.7, "COMMENT"),
        _item("t3", 0.6, "TWEET"),
        _item("c2", 0.5, "COMMENT"),
    ]
    # TWEET then COMMENT interleaved: t1, c1, t2, c2, t3
    result = ranking.apply_type_quotas(items, required_types=["TWEET", "COMMENT"])
    assert [r.content_id for r in result] == ["t1", "c1", "t2", "c2", "t3"]


def test_type_quotas_no_op_when_none() -> None:
    items = [_item("a", 0.9, "TWEET"), _item("b", 0.8, "COMMENT")]
    assert ranking.apply_type_quotas(items, required_types=None) == items


def test_type_quotas_drops_off_type_items() -> None:
    items = [
        _item("t1", 0.9, "TWEET"),
        _item("article", 0.8, "ARTICLE"),  # not in required_types — must drop
        _item("c1", 0.7, "COMMENT"),
    ]
    result = ranking.apply_type_quotas(items, required_types=["TWEET", "COMMENT"])
